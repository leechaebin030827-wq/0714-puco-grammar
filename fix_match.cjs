...
const replacement = `  const translateToEn = (text) => {
  ...
  function matchLocalScenario(inputText) {
    ...
    const isToFTrigger = ["거리", "가깝", "가까이", "멀어", "다가오", "이탈", "접근", "cm", "m", "초점", "스케일", "재질", "평탄"].some(w => text.includes(w));
    ...
    for (const item of capabilityDb.SN) {
      ...
      if (isToFCode && !isToFTrigger && score < 18) continue; 
      if (!isToFCode && !isCameraTrigger && score < 18) continue;
      if (score >= 8.0) matchedSN.push({ code: item.code, score: score });
    }

    for (const item of capabilityDb.MP) {
      if (item.enabled === false) continue;
      const score = getMatchScore(text, item);
      if (score >= 8.5) matchedMP.push({ code: item.code, score: score });
    }
    ...
    if (isProjectionNeeded) {
      for (const item of capabilityDb.PJ) {
        if (item.enabled === false) continue;
        const score = getMatchScore(text, item);
        if (score >= 8.5) matchedPJ.push({ code: item.code, score: score });
      }
    }
    ...
    if (isSpeakerNeeded) {
      for (const item of capabilityDb.SP) {
        if (item.enabled === false) continue;
        const score = getMatchScore(text, item);
        if (score >= 8.5) matchedSP.push({ code: item.code, score: score });
      }
    }
...
const endIndex = content.indexOf('  const apiKey = process.env.ANTHROPIC_API_KEY;');

if (startIndex !== -1 && endIndex !== -1) {
  content = content.substring(0, startIndex) + replacement + '\n' + content.substring(endIndex);
  fs.writeFileSync('/Users/yoon/Desktop/PUCO/api/match.js', content, 'utf8');
  console.log('Successfully replaced corrupted text.');
} else {
